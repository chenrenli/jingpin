...
class MemberFeed extends Base{
    public function index(){
        $memberFeedModel = new \app\common\model\MemberFeed();
        $map = array();
        $member_feed = $memberFeedModel->where($map)->paginate(10);
    ...
    public function delete(){
        if($this->request->isAjax()){
            ...
            $map['id'] = array("in",$id);
            $result = \app\common\model\MemberFeed::destroy($map);
            if($result){
